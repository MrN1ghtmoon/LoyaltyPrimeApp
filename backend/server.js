...
app.post('/api/users/completeQuest', async (req, res) => {
    try {
        ...
        if (!isCompleted) {
            return res.status(400).json({ 
                error: `������� ��� �� ���������. ��������: ${currentProgress}/${targetValue}` 
            });
        }
        ...
        // ��������� ����� (������ �����!)
        await updateUserBalance(userId, reward, 'earn', `������� ���������! +${reward} �������`);
...
app.get('/api/users/:userId/promotions/purchased/:companyId', async (req, res) => {
    try {
        ...
        console.log(`?? ���������� ${purchasedWithCycle.length} ��������� ����� ��� ������������ ${userId}`);
...
app.post('/api/pos/use-purchased-promotion', async (req, res) => {
    try {
        ...
        console.log('🔍 [DEBUG] Получен запрос на использование акции:', { promotionId, promotionCycleStart });
        ...
        console.log('👤 Пользователь:', { id: user.id, vkId: user.vk_id });
        ...
        // 🔍 Сначала посмотрим все неиспользованные записи пользователя по этой акции
        ...
        console.log('📊 Все неиспользованные записи в БД:', allUnused.rows);
        console.log('📊 Ищем с promotionCycleStart:', promotionCycleStart);
        ...
        if (promotionCycleStart) {
            ...
            console.log('🔍 Результат поиска с promotionCycleStart:', checkResult.rows);
        }
        
        // Если не нашли по cycle_start, пробуем найти любую неиспользованную
        if (!checkResult || checkResult.rows.length === 0) {
            console.log('⚠️ Не найдено по cycle_start, ищем любую неиспользованную...');
            checkResult = await query(
            ...
            console.log('🔍 Результат поиска любой неиспользованной:', checkResult.rows);
        }
        ...
        console.log('✅ Акция успешно использована! Осталось:', remaining);
...
app.delete('/api/promotions/:id', async (req, res) => {
    try {
        ...
        console.error('? ������ �������� �����:', error);
        res.status(500).json({ success: false, error: error.message, message: '������ ������� ��� �������� �����' });
    }
});
